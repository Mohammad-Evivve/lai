...
const L4_WORKER_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY;

/**
 * Asynchronously triggers the Layer 4 Feature Extraction worker.
 * Fire-and-forget to maintain API performance.
 */
function triggerFeatureExtraction(table, record) {
  ...
  // Use axios to fire the request without waiting for the response
 ...
 * Role: Catch T1/T2 simulation events.
 */
router.post('/telemetry/ingest', async (req, res) => {
  ...
  try {
    ...
    // 4. TRIGGER LAYER 4 WORKER (Asynchronous)
    if (insertedEvents) {
 ...
 * Role: Catch T3 diagnostic survey data.
 */
